...
import com.blackangel.baframework.media.camera.CameraPictureFileBuilder;
import com.blackangel.baframework.media.camera.fragment.CameraFragment;
...
import com.blackangel.baframework.media.image.view.ImagePickWithCamTitleView;
...
public class BaseImagePickWithCameraActivity extends BaseImagePickActivity implements PreviewsFragment.PreviewActionListener,
    ...
    public void onClick(View v) {
        if(v.getId() == R.id.btn_camera_gallery) {
            ...
            intent.putStringArrayListExtra(IntentConst.KEY_IMAGE_PATH_LIST, mPreviewsFragment.getPreviewImagePathList());
...
//            }
            ...
            startActivityForResult(intent, IntentConst.REQUEST_EDIT_IMAGE);
        }
    }
...
//                }.sendEmptyMessageDelayed(0, 500);
    ...
    public void onAddedPreview(PreviewView previewView) {
        ...
        MyLog.d("previewView=" + previewView);
    ...
    public void onRemovedPreview(int removeIndex, PreviewsFragment.PreviewData previewData, boolean isNeedGalleryDeselect) {
        MyLog.d("removeIndex=" + removeIndex + ", previewData=" + previewData);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == IntentConst.REQUEST_ZOOM_IMAGE) {
            if (resultCode == RESULT_CANCELED) {
                // 삭제된 이미지 패스 리스트 업데이트
                ArrayList<String> delImagePathList = data.getStringArrayListExtra(IntentConst.KEY_IMAGE_PATH_LIST);
                MyLog.d("delImagePathList=" + delImagePathList);

                updateSelectedImage(delImagePathList);
            }
        } else if (requestCode == IntentConst.REQUEST_EDIT_IMAGE) {
            if (resultCode == RESULT_OK) {
                // 편집 완료 후 돌아옴 - 편집완료 된 이미지 경로 리스트 전달 후 종료
                ArrayList<String> pathList = data.getStringArrayListExtra(IntentConst.KEY_IMAGE_PATH_LIST);
    ...
    public void onSuccessTakenPictureAndSaveFile(final CameraPictureFileBuilder.BuildImageResult buildImageResult) {
        ...
        if(mGalleryFragment.getGalleryBucketName().equals(MyPackageManager.getPublicAppAlbumName(this))) {
