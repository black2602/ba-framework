...
public class BitmapBuilder extends AsyncTask<String, Void, Bitmap> {
    ...
    protected Bitmap doInBackground(String... params) {
        ...
        MyLog.d("original bitmap size = " + options.outWidth + " x " + options.outHeight);

        options.inJustDecodeBounds = false;
        options.inSampleSize = calculateInSampleSize(options, mDestWidth, mDestHeight);
        MyLog.d("inSampleSize=" + options.inSampleSize);

        Bitmap bitmap = BitmapFactory.decodeFile(imagePath, options);

        if(bitmap != null) {
            MyLog.d("sampling bitmap size = " + bitmap.getWidth() + " x " + bitmap.getHeight());
            ...
            if(rotatedBitmap.getWidth() > mDestWidth) {
                ...
                MyLog.d("scaled bitmap size = " + scaledBitmap.getWidth() + " x " + scaledBitmap.getHeight());
