...
public class MySecurityKeyboard extends LinearLayout implements View.OnClickListener {
    ...
    public void show(final int startFocusIndex, final boolean rearrange) {
        BaLog.d("startFocusIndex = " + startFocusIndex);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        BaLog.i("changed=" + changed + ", " + l + ", " + t + ", " + r + ", " + b);
    ...
    private void arrangeKeys() {
        ...
        for(int i = 0; i < 3; i++) {
            ...
            for(int j = 0; j < 4; j++) {
//                BaLog.d("i=" + i + ", j=" + j + ", rand=" + rand + ", num=" + num);
...
//                    BaLog.d("키 생성 = " +  mBtnRows[i][j].getTag() + ", num=" + num);
                }
...
//        debugKeyModels();
    ...
    private void bindModel(Button button, int num) {
        ...
        BaLog.d(num + " button area = " + model.getArea().toString());
...
//        BaLog.d("x=" + x + ", y=" + y);
        ...
        for (MySecurityKeyboardModel model : mKeyModels) {
//            BaLog.d("model.getArea()=" + model.getArea().toString());

            if(model.getArea().contains(x, y)) {
                BaLog.i("found input key model!! > " + model);
                return model;
            }
        }
...
//                    btn.onTouchEvent(ev);
    ...
    private Button getPressedButton(Rect area, int x, int y) {
        ...
        for (Button[] btnRow : mBtnRows) {
            for (Button btn : btnRow) {
                ...
                if(r.contains(x, y)) {
                    BaLog.i("found touched Button >> " + btn.getText().toString());
                    return btn;
                }
            }
        }
    ...
    private void inputKey(MySecurityKeyboardModel touchedModel) {
        BaLog.i("touchedModel = " + touchedModel);

        for(int i=0; i < mEditTexts.length; i++) {
            EditText editText = mEditTexts[i];

            BaLog.d("editText " + i + " hasFocus = " + editText.hasFocus());
...
//    }
    ...
    public String getEncryptedInputValuesToServer() {
        ...
        for(int i=0; i < mInputedKeyModels.length; i++) {
            ...
            BaLog.d("i=" + i + ", inputKeyModel = " + inputedKeyModel);
        ...
        BaLog.i("twoDepthEncValue = " + twoDepthEncValue);
